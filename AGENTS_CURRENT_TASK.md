...
- `squalr-engine-api` process contracts (`ProcessInfo`, `OpenedProcessInfo`) no longer expose `sysinfo::Pid`; they now expose plain `u32` IDs so OS-specific PID types stay in `squalr-operating-system`/session layers.
...
- 2026-02-09: Removed `sysinfo::Pid` from `squalr-engine-api` process surfaces by changing `ProcessInfo`/`OpenedProcessInfo` PID accessors to `u32`, moved PID conversion back into `squalr-operating-system` process manager, updated GUI call sites accordingly, and validated with `cargo fmt --all`, `cargo check -p squalr-engine-api -p squalr-operating-system -p squalr-engine-session -p squalr-engine -p squalr -p squalr-cli -p squalr-tests`, and `cargo test -p squalr-engine-api -p squalr-operating-system -p squalr-engine-session -p squalr-engine -p squalr-cli -p squalr-tests --no-run`.
