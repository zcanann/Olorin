...
- Session checkpoint (2026-02-16, follow-up): Re-ran `cargo test -p squalr-tests --test project_items_command_tests` (19 passed) and `cargo test -p squalr-tests --test scan_results_command_tests` (20 passed); repository remains clean and no new concrete `pr/project-explorer` regressions were identified.
- Session checkpoint (2026-02-16, follow-up): Audited `pr/project-explorer` TODO/JIRA/stub markers across hierarchy/project-items/scan-results command paths (no new concrete regressions), then ran `cargo test -p squalr-tests --test project_items_command_tests` (19 passed) and `cargo test -p squalr-tests --test scan_results_command_tests` (20 passed).
...
- Session checkpoint (2026-02-16, follow-up): Re-audited `pr/project-explorer` TODO/JIRA/stub markers (no new concrete regressions to queue; only existing deferred `project save` JIRA note), then ran `cargo test -p squalr-tests --test project_items_command_tests` (19 passed) and `cargo test -p squalr-tests --test scan_results_command_tests` (20 passed); repository remains clean.
- Fixed `ProjectItemTypeAddress::get_field_address` 4-byte decode path: now deserializes stored bytes instead of always returning `0`, preserving legacy/32-bit address values in project explorer edits and previews.
...
- Removed `TODO` placeholder-name normalization from address/pointer project-item constructors; default names now apply only when input is blank.
- Removed remaining hardcoded `"TODO"` runtime string placeholder from `ValuedStructField::get_display_string` value branch (now empty string fallback) and ran `cargo fmt`, `cargo test -p squalr-engine-api project_item_type_address` (4 passed), `cargo test -p squalr-engine-api project_item_type_pointer` (2 passed), and `cargo test -p squalr-engine-domain valued_struct_field` (0 tests, pass).
