...
- [ ] Move project-item mutation flows to unprivileged command routing.
- [ ] Replace privileged `scan-results add-to-project` path with an unprivileged project-items add command, then remove the stubbed privileged executor path.
- [ ] Expand `project-items` command surface beyond `list`/`activate` to support create, delete, rename, move, and reorder operations needed by project explorer UX.
