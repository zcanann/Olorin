...
### Phase 2 (do now): OS Mock / DI seam
...
- `cargo test -p squalr-tests` is currently passing (97 integration tests).
- OS-behavior tests are **blocked** until privileged OS singletons support DI overrides.
...
- Create a single mocked OS API surface in `squalr-tests` (one module, one source of truth). There are already traits like `IMemoryReader`, `IMemoryWriter`, `IMemoryQueryer`, and `ProcessQueryer`. We would need to create mock/DI compatible usages of these traits.
- Add a minimal DI seam (preferred: trait-object providers hung off `EnginePrivilegedState`, with production default bound to current singletons).
- Write the first OS-behavior tests for:
   - memory read/write,
   - memory range query,
   - scan flows that rely on OS results.
- If DI seam expansion gets ugly, cut scope: prioritize the smallest seam that unblocks deterministic tests.

### Architecture Plan (Modify sparringly as new information is learned. Keep minimal and simple)
- Phase 1: command parsing + request dispatch + typed response decode via engine API mocks. ✅
- Phase 2: OS-behavior tests once privileged OS access is injectable. 🔜
- Proposed seam: attach process/memory/query providers to `EnginePrivilegedState` (trait objects), defaulting to current implementations in production.
...
- `pr/unit-tests`: Added broad parser + command/response contract coverage (97 tests passing).
- `pr/unit-tests`: Phase 2 deferred pending OS singleton DI seams.
