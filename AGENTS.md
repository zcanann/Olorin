...
- `pr/unit-tests`: Revalidated `cargo test -p squalr-tests` (all passing) and reconfirmed static singleton usage under `scan_results` is limited to intentional `add_to_project` stub path.
